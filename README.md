...
옷 사진 한 장을 받으면 ① 가상 피팅으로 착용샷 광고 소재를 대량 생성하고 ② 인스타/네이버에
소액 분산 집행한 뒤 ③ 성과 좋은 소재는 변형 증식, 나쁜 소재는 자동 오프하며 예산을
...
- `POST /api/creatives/generate` — Gemini 착용 소재 생성 및 저장
- `POST /api/campaigns` — 캠페인 설정과 생성 소재 연결
...
2. **소재 생성** — 모델 4종 × 배경 3종 × 카피 2종 = 24개 소재 그리드 (순차 페이드인)
3. **집행 설정** — 일 예산 슬라이더(1~5만 원) · 타겟 칩 · 채널 토글
4. **성과 대시보드** — 7일 타임랩스 재생: KPI 타일 / ROAS 추이 차트 / 소재별 예산 배분 /
   소재 현황 미니뷰 / 이번 주 성적표

## 소재 이미지 교체법 (Fliption 실제 생성 이미지로)
...
- 번호 규칙: 모델(캐주얼→스트릿→오피스→플러스) × 배경(스튜디오→카페→스트릿) × 카피(A→B)
  ...
  `c24`=플러스·스트릿·B
- 권장 비율 3:4 (세로형). 파생 소재(v1~v3)는 부모 소재의 이미지를 공유합니다.

## 시뮬레이션 로직 요약 (`src/lib/simulate.ts`)

"시뮬레이션이지만 알고리즘은 실제"가 포인트입니다.

- **숨은 실제 성과**: 24개 소재에 true CTR 0.3%~2.8%를 하드코딩 (소수 승자 구조,
  `src/lib/creatives.ts`의 `CTR_TABLE`). CVR은 CTR과 약한 양의 상관.
- **톰슨 샘플링**: 소재별 클릭 성과를 Beta(1+클릭, 1+노출−클릭) 사후분포로 유지하고,
  매일 500회 샘플 대결의 승률로 예산 점유율을 결정 (활성 소재당 최소 점유율 보장으로
  탐색 유지).
- **판정 규칙**: Day 1은 균등 분산(탐색) → Day 3 종료 시 관측 CTR 하위 30% 자동 오프 →
  Day 4부터 관측 ROAS 기반 추가 오프 + 상위 소재의 **변형 3종 증식** (Day 4·5, 부모의
  관측 성과를 축소해 사전분포로 상속).
- **광고 피로도**: 한 소재에 예산이 몰릴수록 효과 CTR이 감쇠 — 승자 독식 폭주를 막는
  현실적 장치이자 밴딧이 균형점을 찾게 하는 메커니즘.
- **재현성**: 모든 난수는 시드 고정 mulberry32(`src/lib/prng.ts`). 기본 시드 137은
  "Day 1 ROAS ≈ 0.8x → Day 7 ≈ 2.1x 단조 상승, 베스트 소재 = 스트릿×카페×카피B 계열"
