# FitLoop (핏루프) — GRAFFITI 2026 프로젝트 포트폴리오

옷 사진 한 장을 받으면 ① 가상 피팅으로 착용샷 광고 시안을 대량 생성하고 ② 인스타/네이버에
소액 분산 집행한 뒤 ③ 성과 좋은 시안은 변형 증식, 나쁜 시안은 자동 오프하며 예산을
재배분하는 루프를 자동화하는 서비스의 발표용 데모 웹앱입니다.

현재 공개 사이트는 GitHub Pages에서 제공하는 **정적 포트폴리오 + 브라우저 BYOK 데모**입니다. 첫 화면에서
프로젝트 배경·검증 결과와 같은 도메인에 배포한 발표 덱·A2 포스터 PDF를 확인할 수 있고,
인터랙티브 데모는 키가 없을 때 미리 준비된 광고 이미지로 동작합니다. 사용자가 자신의 Gemini
API 키를 연결하면 브라우저가 Google Gemini API를 직접 호출해 카피와 이미지 12종을 생성합니다.
**예산 배분 알고리즘(톰슨 샘플링)은 브라우저에서 실제로 계산**됩니다.

프로덕션: <https://fitloop.jaeyeong2026.com>

## 실행법

```bash
npm install
VITE_STATIC_DEPLOYMENT=true npm run dev
```

프로덕션 빌드 확인:

```bash
npm run build
npm run test:api
npm run qa:ui
```

## GitHub Pages 배포

`main` 브랜치에 push하면 `.github/workflows/pages.yml`이 GitHub Pages용 프론트엔드를
빌드하고 배포합니다. 워크플로는 `VITE_STATIC_DEPLOYMENT=true`만 설정하며 API 주소나 비밀값을
주입하지 않습니다. 정적 빌드에서는 환경에 오래된 FitLoop API 주소가 남아 있어도 무시합니다.

## 브라우저 API 키 (BYOK)

- 키는 `fitloop:gemini-api-key`라는 이름으로 현재 Origin의 `localStorage`에만 저장합니다.
- 키는 URL·번들·GitHub·FitLoop 서버·콘솔 로그에 포함하지 않습니다.
- 검증과 생성 요청에만 `x-goog-api-key` 헤더로 `generativelanguage.googleapis.com`에 직접 전송합니다.
- 키를 연결하지 않으면 Google API 요청 없이 기존 예시 이미지 데모가 동작합니다.
- 생성 이미지 12개는 현재 탭 메모리에만 유지하며 영구 업로드하지 않습니다.
- 설정 창의 `키 삭제`로 브라우저 저장 키를 즉시 지울 수 있습니다.
- CSP는 자체 번들 외 스크립트를 차단하므로 Cloudflare가 자동 삽입하는 Insights beacon도 실행되지 않습니다.

Google은 운영 웹에 클라이언트 키를 노출하지 말고 백엔드 프록시를 쓰는 방식을 권장합니다. 이
프로젝트는 사용자가 자신의 키를 명시적으로 넣는 데모용 BYOK이므로, 개인 기기에서만 사용하고
Google Cloud의 HTTP 리퍼러 제한과 사용량·결제 알림을 설정해야 합니다.

## 실험용 백엔드

`server.mjs`와 API 테스트는 개발 기록 및 로컬 실험용으로 보존합니다. 공개 GitHub Pages 사이트는
아래 엔드포인트에 연결하지 않습니다.

- `GET /api/health` — 서버 및 Gemini 설정 상태
- `POST /api/products` — 파일 업로드 또는 공개 URL의 상품 이미지 저장
- `POST /api/copies/generate` — 상품·포즈·배경별 광고 카피 12개 자동 생성
- `POST /api/creatives/generate` — Gemini 착용 시안 생성 및 저장
- `POST /api/campaigns` — 캠페인 설정과 생성 시안 연결

앱 자체의 일일 이미지 생성 한도는 두지 않습니다. 카피 12개는 Gemini 구조화 출력으로 한 번에
작성하고, 이미지 12개는 동시에 병렬 생성합니다. 서버는 동시 완료된 결과를 저장 큐로 안전하게
직렬 기록합니다. 단, Google 계정의 결제 잔액과 Gemini API 공급자 쿼터는 그대로 적용됩니다.

저장 데이터는 `data/` 아래에 기록되며 Git에는 포함되지 않습니다. 구매 페이지 URL을 넣으면
Open Graph, Product JSON-LD, Twitter Card, 동적 스크립트, 대표/지연 로딩 이미지, CSS 배경
순으로 후보를 찾아 첫 번째 유효한 JPG·PNG·WebP 이미지와 상품명·가격을 저장합니다. 쿠팡 상품 및 단축 링크는
공식 파트너스 API 검색 결과에서 URL의 상품 번호와 정확히 일치하는 상품만 선택합니다. 모든 리다이렉트 단계에서 사설 IP 접근을 다시
차단하고, 업로드 및 원격 이미지는 8MB로 제한합니다. 로그인·캡차가 필요하거나 브라우저에서
자바스크립트를 실행해야만 이미지가 나타나는 쇼핑몰은 가져오기가 제한될 수 있습니다.

### 발표 리허설 팁

- `?step=4` 처럼 URL 파라미터로 특정 단계로 바로 진입할 수 있습니다 (1~5).
- `?step=5&day=7` 로 대시보드의 특정 일자를 바로 볼 수 있습니다 (1~7).

## 화면 구성 (5단계 위저드)

1. **상품 입력** — 구매 페이지 URL 또는 상품 이미지 → 대표 이미지·상품명·가격 준비
2. **타겟·예산** — 일 예산 슬라이더(1~5만 원) · 성별·연령 타겟 · 채널 토글
3. **모델 선택** — 성별·나이대·피부색·인종·체형 필터 · Fliption 가상 모델 최대 4명 선택
4. **광고 시안** — 키 미연결 시 예시 이미지 12개, 키 연결 시 Nano Banana 2 실생성 12개
5. **성과 대시보드** — 7일 타임랩스 재생: KPI 타일 / ROAS 추이 차트 / 시안별 예산 배분 /
   시안 현황 미니뷰 / 이번 주 성적표

## 광고 시안 이미지 교체법 (Fliption 실제 생성 이미지로)

`/public/creatives/` 폴더에 `c01.jpg` ~ `c12.jpg` 파일을 넣으면 플레이스홀더 대신
**해당 이미지가 자동으로 우선 사용**됩니다. 파일이 없으면 듀오톤 배경 + 실루엣
플레이스홀더가 그려집니다.

- 번호 규칙: 포즈/모델 슬롯(1→4) × 배경(스튜디오→카페→스트릿) 순서.
  즉 `c01`=포즈1·스튜디오, `c02`=포즈1·카페, `c03`=포즈1·스트릿, …
  `c12`=포즈4·스트릿
- 권장 비율 3:4 (세로형). 파생 시안(v1~v3)은 부모 시안의 이미지를 공유합니다.

## 시뮬레이션 로직 요약 (`src/lib/simulate.ts`)

"시뮬레이션이지만 알고리즘은 실제"가 포인트입니다.

- **숨은 실제 성과**: 12개 시안에 true CTR 0.4%~2.8%를 하드코딩 (소수 승자 구조,
  `src/lib/creatives.ts`의 `CTR_TABLE`). CVR은 CTR과 약한 양의 상관.
- **톰슨 샘플링**: 시안별 클릭 성과를 Beta(1+클릭, 1+노출−클릭) 사후분포로 유지하고,
  매일 500회 샘플 대결의 승률로 예산 점유율을 결정 (활성 시안당 최소 점유율 보장으로
  탐색 유지).
- **판정 규칙**: Day 1은 균등 분산(탐색) → Day 3 종료 시 관측 CTR 하위 30% 자동 오프 →
  Day 4부터 관측 ROAS 기반 추가 오프 + 상위 시안의 **변형 3종 증식** (Day 4·5, 부모의
  관측 성과를 축소해 사전분포로 상속).
- **광고 피로도**: 한 시안에 예산이 몰릴수록 효과 CTR이 감쇠 — 승자 독식 폭주를 막는
  현실적 장치이자 밴딧이 균형점을 찾게 하는 메커니즘.
- **재현성**: 모든 난수는 시드 고정 mulberry32(`src/lib/prng.ts`). 기본 시드 137은
  "Day 1 탐색 → Day 7 승자 집중, 베스트 시안 = 포즈2×카페 계열"
  조건을 예산 1~5만 원 전 구간에서 만족하도록 시드 스캔으로 선정했습니다.
- 매출은 기대 전환 기반(+소폭 노이즈)으로 계산해, 소액 집행에서 정수 전환의 산탄
  노이즈를 걷어내고 예산 재배분 역학이 그래프에 그대로 드러나게 했습니다.

## 스택

Vite · React 19 · TypeScript · Tailwind CSS v4 · Recharts

디자인 토큰은 fliption.co.kr 기준: 텍스트 `#030712`, 보더 `#e5e7eb`, 액센트 `#f97316`,
Inter Tight + Pretendard.
